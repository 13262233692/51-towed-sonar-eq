...
std::vector<std::complex<double>> generate_step_transient_signal(
    ...
    std::vector<std::complex<double>> transmit(num_samples);
    for (size_t i = 0; i < num_samples; ++i) {
        double gain = (i < step_position) ? pre_gain : post_gain;
        double phi = phase_dist(rng);
        transmit[i] = std::polar(modulus * gain, phi);
    }

    size_t mp = std::max(size_t(1), multipath_taps);
    std::vector<std::complex<double>> channel(mp);
    double total = 0.0;
    for (size_t k = 0; k < mp; ++k) {
        double mag = std::exp(-0.25 * k) * (0.7 + 0.5 * std::abs(gauss(rng)));
        double ph = 0.6 * k + gauss(rng) * 0.15;
        channel[k] = std::polar(mag, ph);
        total += mag;
    }
    for (auto& c : channel) c /= total;
...
static void int32_to_int24_local(int32_t val, uint8_t* out) {
    val = std::clamp(val, -sonar::INT24_MAX - 1, sonar::INT24_MAX);
    out[0] = static_cast<uint8_t>(val & 0xFF);
    out[1] = static_cast<uint8_t>((val >> 8) & 0xFF);
    out[2] = static_cast<uint8_t>((val >> 16) & 0xFF);
...
static uint16_t local_crc16(const uint8_t* data, size_t len) {
    uint16_t crc = 0xFFFF;
    for (size_t i = 0; i < len; ++i) {
        crc ^= static_cast<uint16_t>(data[i]);
        for (int j = 0; j < 8; ++j) {
...
std::vector<uint8_t> generate_thermocline_stream(
    ...
    const uint64_t dt_ns = static_cast<uint64_t>(1e9 / sample_rate_hz);
    const uint64_t start_ts = 1800000000000000000ULL;
    ...
    for (size_t f = 0; f < num_frames; ++f) {
        ...
        pkt.timestamp_ns = start_ts + f * dt_ns;
        pkt.packet_sequence = static_cast<uint32_t>(f);

        double depth;
        if (f < step_frame) {
            depth = shallow_depth_m;
        } else if (f < step_frame + 50) {
            double t = static_cast<double>(f - step_frame) / 50.0;
            depth = shallow_depth_m + (deep_depth_m - shallow_depth_m) * t;
        } else {
            depth = deep_depth_m;
        }
        pkt.pressure_raw = static_cast<uint16_t>(depth / sonar::PRESSURE_SCALE);

        double t_frame = static_cast<double>(f) / sample_rate_hz;
        for (size_t ch = 0; ch < sonar::NUM_HYDROPHONE_CHANNELS; ++ch) {
            double amplitude;
            if (f < step_frame) {
                amplitude = pre_amplitude;
            } else if (f < step_frame + 30) {
                double spike_t = static_cast<double>(f - step_frame) / 30.0;
                double envelope = std::sin(M_PI * spike_t);
                amplitude = pre_amplitude + (post_amplitude_spike - pre_amplitude) * envelope;
            } else {
                amplitude = post_amplitude_spike * 0.3;
            }

            double phase = 2.0 * M_PI * 850.0 * t_frame
                         + static_cast<double>(ch) * 0.08;
            double voltage = amplitude * std::sin(phase) + noise(rng);

            int32_t raw = static_cast<int32_t>(voltage / sonar::VOLTAGE_SCALE);
            int32_to_int24_local(raw, pkt.channel_data + ch * 3);
        }

        const size_t crc_offset = offsetof(sonar::RawTelemetryPacket, crc16);
        pkt.crc16 = local_crc16(
            reinterpret_cast<const uint8_t*>(&pkt), crc_offset);
